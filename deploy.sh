...
#
# The refresh-reviews GitHub Action commits regenerated HTML to main every
# time you deploy (fresh Google reviews + rebuilt pages). By the time you
# deploy again, remote is one commit ahead and every rendered HTML file
# differs — because both your zip AND the workflow ran build.py against
# slightly different inputs. A naive `git push` is rejected; a naive
# `git pull --rebase` conflicts on every page.
#
# Fix: rebase with `-X theirs` merge strategy option. In rebase context
# "theirs" = the commits being replayed = YOUR incoming build. So git will
# auto-resolve every conflict by keeping your fresh build. Anything stale
# (e.g. review count) will be regenerated by the workflow that auto-fires
# ~60 seconds after this push lands. Net effect: your deploy always wins
# the source-code race, and the workflow always wins the data race.
# ---------------------------------------------------------------------------
echo ""
...
for attempt in 1 2 3; do
  ...
  echo ""
  echo "⚠ Push rejected (remote moved). Rebasing with -X theirs (prefer your incoming build)…"

  if ! git pull --rebase -X theirs origin main; then
    echo ""
    echo "❌ Rebase failed even with automatic conflict resolution."
    echo "   This usually means an add/delete conflict that -X can't resolve."
    echo "   Resolve manually:"
    echo "     git status                              # see which files conflict"
    echo "     # For each conflicted file:"
    echo "     git checkout --theirs <file>            # keep YOUR incoming version"
    echo "     # OR"
    echo "     git checkout --ours <file>              # keep remote version"
    echo "     git add <file>"
